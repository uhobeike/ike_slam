...
#ifndef IKE_SLAM__RESAMPLING_HPP_
#define IKE_SLAM__RESAMPLING_HPP_
...
public:
  ...
  void systematicSampling(
      std::vector<Particle> &particles); // 系統サンプリングをする
  double calculateSystematicSamplingStart(
      double random_max); // 系統サンプリング用のランダムなstartを計算する
  double
  calculateSystematicSamplingStep(); // 系統サンプリング用のstepを計算する
  void normalize(
      std::vector<Particle> &particles); // パーティクルの重みを正規化をする
