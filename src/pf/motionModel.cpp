...
namespace mcl {
MotionModel::MotionModel(double alpha_trans_trans, double alpha_trans_rotate,
    ...
    : alpha_trans_trans_(alpha_trans_trans),
      ...
      alpha_rotate_rotate_(alpha_rotate_rotate), engine_(seed_gen_()) {
  std::cerr << "Done MotionModel::MotionModel."
            << "\n";
};
MotionModel::~MotionModel(){};

// Probabilistic Robotics p.124
void MotionModel::update(std::vector<Particle> &particles,
                         double current_pose_yaw, double delta_x,
                         double delta_y, double delta_yaw) {
  ...
  delta_rotate_1_noise = std::min(fabs(diffMinAngle(delta_rotate_1, 0.)),
                                  fabs(diffMinAngle(delta_rotate_1, M_PI)));
  delta_rotate_2_noise = std::min(fabs(diffMinAngle(delta_rotate_2, 0.)),
                                  fabs(diffMinAngle(delta_rotate_2, M_PI)));

  for (auto &p : particles) {
    delta_rotate_1_hat = diffMinAngle(
        delta_rotate_1,
        drawNoise(sqrt(alpha_trans_trans_ * delta_rotate_1_noise *
                           delta_rotate_1_noise +
                       alpha_trans_rotate_ * delta_trans * delta_trans)));
    delta_trans_hat =
        delta_trans -
        drawNoise(sqrt(alpha_rotate_trans_ * delta_trans * delta_trans +
                       alpha_rotate_rotate_ * delta_rotate_1_noise *
                           delta_rotate_1_noise +
                       alpha_rotate_rotate_ * delta_rotate_2_noise *
                           delta_rotate_2_noise));
    delta_rotate_2_hat = diffMinAngle(
        delta_rotate_2,
        drawNoise(sqrt(alpha_trans_trans_ * delta_rotate_2_noise *
                           delta_rotate_2_noise +
                       alpha_trans_rotate_ * delta_trans * delta_trans)));

    p.pose.position.x +=
        delta_trans_hat * cos(p.pose.euler.yaw + delta_rotate_1_hat);
    p.pose.position.y +=
        delta_trans_hat * sin(p.pose.euler.yaw + delta_rotate_1_hat);
    p.pose.euler.yaw = normalizeAngle(p.pose.euler.yaw) + delta_rotate_1_hat +
                       delta_rotate_2_hat;
  }
...
double MotionModel::diffMinAngle(double angle1, double angle2) {
  ...
  return (fabs(diff_angle_1) < fabs(diff_angle_2)) ? diff_angle_1
                                                   : diff_angle_2;
}

void MotionModel::getDelta(double &delta_x, double &delta_y, double &delta_yaw,
                           double current_x, double past_x, double current_y,
                           double past_y, double current_yaw, double past_yaw) {
